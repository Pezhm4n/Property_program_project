...
class SettingsDialog(QDialog):
    ...
    def select_font(self):
        ...
        try:
            ...
            self.logger.debug(f"انتخاب فونت: فونت فعلی = {current_font.family()}")
            
            # اول تلاش می‌کنیم ببینیم آیا getFont یک tuple برمی‌گرداند
            result = QFontDialog.getFont(current_font, self, "انتخاب فونت برنامه")
            self.logger.debug(f"نتیجه دیالوگ فونت: نوع={type(result)}")
            
            # روش 1: بررسی بر اساس نوع
            if isinstance(result, tuple) and len(result) == 2:
                ok, font = result
                if ok and font:
                    # اگر ok باشد، font را استفاده می‌کنیم
                    font_str = f"{font.family()}, {font.pointSize()}pt"
                    self.logger.debug(f"فونت انتخاب شده (روش 1): {font_str}")
                    self.font_label.setText(font_str)
                    self.font_label.setFont(font)
                    self.font_label.setProperty("selectedFont", font)
                return
            
            # روش 2: نسخه‌های قدیمی‌تر ممکن است getFont مستقیماً یک فونت برگرداند
            try:
                # بررسی می‌کنیم آیا result یک شیء QFont است
                if hasattr(result, 'family') and hasattr(result, 'pointSize'):
                    font_str = f"{result.family()}, {result.pointSize()}pt"
                    self.logger.debug(f"فونت انتخاب شده (روش 2): {font_str}")
                    self.font_label.setText(font_str)
                    self.font_label.setFont(result)
                    self.font_label.setProperty("selectedFont", result)
                    return
            except Exception as e:
                self.logger.error(f"خطا در روش 2: {str(e)}")
            
            # روش 3: برای نسخه‌های دیگر PyQt که ممکن است رفتار متفاوتی داشته باشند
            # به عنوان آخرین راه حل، از دیالوگ انتخاب فونت سفارشی استفاده می‌کنیم
            self.logger.debug("استفاده از روش 3 (جایگزین) برای انتخاب فونت")
            try:
                dialog = QDialog(self)
                dialog.setWindowTitle("انتخاب فونت")
                ...
                layout.addWidget(QLabel("انتخاب فونت:"))
                ...
                if dialog.exec_() == QDialog.Accepted:
                    ...
                    font_str = f"{font.family()}, {font.pointSize()}pt"
                    self.logger.debug(f"فونت انتخاب شده (روش 3): {font_str}")
                    self.font_label.setText(font_str)
                    self.font_label.setFont(font)
                    self.font_label.setProperty("selectedFont", font)
            except Exception as e:
                self.logger.error(f"خطا در روش 3: {str(e)}")
                QMessageBox.warning(self, "خطا", f"خطا در انتخاب فونت: {str(e)}")
                
        except Exception as e:
            # در صورت خطا، آن را در لاگ ثبت می‌کنیم
            self.logger.error(f"خطا در انتخاب فونت: {str(e)}")
            QMessageBox.warning(self, "خطا", f"خطا در انتخاب فونت: {str(e)}")
