...
class ReportTab(QWidget):
    ...
    def __init__(self, current_user, report_generator=None, parent=None):
        super().__init__(parent)
    ...
    def generate_chart(self):
        ...
        try:
            ...
            if report_type == 0:  # تعداد املاک
                ...
                # حذف ردیف‌های با مقدار 'Total'
                if 'Total' in data.iloc[:, 0].values:
            ...
            # تنظیم پارامترهای خاص هر نوع نمودار
            if chart_type == "bar":
                chart_kwargs.update({
                    "x_column": data.columns[0],
                    "y_column": data.columns[1] if len(data.columns) > 1 else None,
                    "xlabel": "دسته‌بندی",
                    "ylabel": "تعداد" if report_type in [0, 2, 3] else "ارزش (تومان)",
                    "show_values": True,
                    "rotation": 45 if report_type in [2, 3] else 0,
                    "grid": True
                })
            elif chart_type == "pie":
                chart_kwargs.update({
                    "labels_column": data.columns[0],
                    "values_column": data.columns[1],
                    "shadow": True,
                    "startangle": 90
                })
