...
class ReportGenerator:
    ...
    def generate_price_range_report(self, property_type: str = 'all', bins: int = 5,
        ...
        Args:
            property_type (str, optional): نوع ملک ('residential', 'commercial', 'land', 'all')
            bins (int, optional): تعداد بازه‌های قیمتی
            output_format (str, optional): فرمت خروجی ('text', 'csv', 'dataframe')
            output_file (str, optional): نام فایل خروجی
        ...
        Returns:
            Union[str, pd.DataFrame]: گزارش به فرمت متنی یا دیتافریم pandas
        """
        # فقط برای املاک فروشی
        properties = self._get_all_properties(deal_type=1)
        ...
        # ایجاد گزارش توصیفی
        property_type_str = property_type.capitalize() if property_type != 'all' else 'All'
        report_title = f"Price Range Report - {property_type_str} Properties (Sale)"
        ...
        if output_format == 'dataframe':
            ...
            if output_file is None:
                output_file = os.path.join(self.output_dir, 
                                         f"price_range_report_{property_type}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
            ...
            if output_file is None:
                output_file = os.path.join(self.output_dir, 
                                         f"price_range_report_{property_type}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
