...
from residential_bridge import ResidentialBridge, ResidentialPropertyStruct
...
class ResidentialTab(QWidget):
    ...
    def register_property(self):
        ...
        try:
            ...
            # ایجاد ساختار داده‌ای ملک مسکونی
            property_data = ResidentialPropertyStruct()
            ...
            property_data.bedrooms = self.bedrooms_spin.value()
            property_data.age = self.age_spin.value()
            property_data.floor = self.floor_spin.value()
            ...
            property_data.has_balcony = 1 if self.has_balcony_check.isChecked() else 0
            property_data.is_furnished = 1 if self.is_furnished_check.isChecked() else 0
            property_data.has_ac = 1 if self.has_ac_check.isChecked() else 0
            
            # ثبت ملک
            success = False
            if deal_type == "sale":
                success = self.residential_bridge.register_sale(property_data)
            else:
                success = self.residential_bridge.register_rental(property_data)
