...
class ChartWidget(QFrame):
    ...
    def set_chart(self, chart_data, chart_params=None):
        ...
        if not chart_data.empty:
            ...
            if chart_params:
                params.update(chart_params)
            
            try:
                if self.chart_type == "bar":
                    self.chart_object = BarChart(**params)
                    self.chart_object.plot(chart_data)
                elif self.chart_type == "pie":
                    self.chart_object = PieChart(**params)
                    self.chart_object.plot(chart_data)
                elif self.chart_type == "line":
                    self.chart_object = LineChart(**params)
                    self.chart_object.plot(chart_data)
...
class DashboardTab(QWidget):
    ...
    def refresh_all(self):
        ...
        try:
            ...
            if not df.empty:
                total = df['تعداد'].sum()
                self.total_properties_card.update_value(total)
                
                # آمار به تفکیک نوع ملک
                for _, row in df.iterrows():
                    if row['نوع ملک'] == 'مسکونی':
                        self.residential_card.update_value(row['تعداد'])
                    elif row['نوع ملک'] == 'تجاری':
                        self.commercial_card.update_value(row['تعداد'])
                    elif row['نوع ملک'] == 'زمین':
                        self.land_card.update_value(row['تعداد'])
