...
def setup_logging(log_level=logging.INFO) -> logging.Logger:
    ...
    Args:
        log_level: سطح گزارش‌دهی (پیش‌فرض: INFO)
        
    Returns:
        آبجکت Logger تنظیم شده
    """
    # تعیین مسیر فایل گزارش
    project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    log_dir = os.path.join(project_dir, "logs")
    os.makedirs(log_dir, exist_ok=True)
    
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    log_file = os.path.join(log_dir, f"property_management_{timestamp}.log")
    
    # ایجاد لاگر
    logger = logging.getLogger("property_management")
    logger.setLevel(log_level)
    
    # بررسی اینکه آیا هندلرها قبلا اضافه شده‌اند
...
def load_config(config_file: str) -> Dict[str, Any]:
    ...
    if os.path.exists(config_file):
        try:
            ...
            logger.info(f"پیکربندی از {config_file} بارگذاری شد")
        ...
        try:
            ...
            logger.info(f"فایل پیکربندی پیش‌فرض در {config_file} ایجاد شد")
        except Exception as e:
            logger.error(f"خطا در ایجاد فایل پیکربندی پیش‌فرض: {e}")
