...
class UserManager:
    ...
    def load_users(self) -> bool:
        ...
        try:
            ...
            # اگر هیچ کاربری وجود نداشته باشد، کاربر admin پیش‌فرض ایجاد کنید
            if not self.users:
                self.create_default_admin()
            ...
            # در صورت خطا در بارگذاری، فایل را دوباره ایجاد کنید
            self.create_default_admin()
            return True
    ...
    def create_default_admin(self) -> None:
        ...
        if not self.users:
            ...
            self.logger.info("کاربر مدیر پیش‌فرض با نام کاربری 'admin' و کلمه عبور 'admin123' ایجاد شد")
