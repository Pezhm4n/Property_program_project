...
def setup_c_functions():
    ...
    if inspect.isclass(type(c_lib)) and hasattr(c_lib, '__class__') and c_lib.__class__.__name__ == 'MockCLib':
        logger.info("از کتابخانه موک استفاده می‌شود - تنظیم توابع C برای زمین نادیده گرفته می‌شود")
        return
    ...
    def get_properties(deal_type="sale"):
        ...
        try:
            ...
            if 'bridge.mock_lib' in sys.modules:
                ...
                properties = [PropertyObject(prop_dict) for prop_dict in mock_data['land'][deal_type_str]]
                
                # برگرداندن داده‌های آماده از mock
                logger.info(f"استفاده از داده‌های mock برای املاک زمین: {len(properties)} ملک یافت شد")
                return properties
